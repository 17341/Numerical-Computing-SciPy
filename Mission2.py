...
for key in sacs.keys():                                                         
    liste1.append(re.search('\d+', key).group())
    for n in range(len(sacs[key])):
        if o.match(sacs[key][n]) is not None:
            liste2.append(re.search('\d+', sacs[key][n]).group())
...
#Solution la plus rapide et la plus efficace:
...
#Solution "plus longue" et moins efficace que les 2 autres.
